...
#             try:
#                 user_metadata = self.users_table.get_all(uid, index='uid').nth(0).run(self.rethink_conn)
#             except r.errors.ReqlNonExistenceError:
...
#             response.update({"crt_u": bytes(crt_u_bytes)})
#             response['agent_cert'] = bytes(response['agent_cert'])
#             response['agent_sig'] = bytes(response['agent_sig'])
#             response['pac'] = bytes(response['pac'])
            
            
#             # Remove the one time keys from the response except the last one:
#             response['one_time_keys'] = [bytes(response['one_time_keys'][-1])]
#             response['one_time_key_sigs'] = [bytes(response['one_time_key_sigs'][-1])]
...
#             logger.log("DEBUG", pprint.pformat(bytes(response['one_time_keys'][0])))
...
class Provider:
    ...
    def _register_routes(self):
        ...
        def access():
            ...
            import pprint
            logger.log("DEBUG", pprint.pformat(t_agent_metadata))
